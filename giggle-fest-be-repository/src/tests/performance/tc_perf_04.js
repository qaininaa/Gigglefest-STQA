...
const customIterationDuration = new Trend("custom_iteration_duration");
  ...
  scenarios: {
    resource_monitoring: {
      ...
      stages: [
        ...
        { duration: "1m", target: 150 }, // Light load
        { duration: "1m", target: 300 }, // Medium load
        { duration: "2m", target: 500 }, // Heavy load (peak capacity)
        { duration: "1m", target: 250 }, // Cool down
...
export default function () {
  ...
  const responses = http.batch([
    ...
    ["GET", `${BASE_URL}/api/v1/tickets`, null, { tags: { name: "tickets" } }],
  ]);
  ...
  customIterationDuration.add(iterationEnd - iterationStart);
...
export function handleSummary(data) {
  ...
  const avgIterationDuration = metrics.iteration_duration?.values.avg || 0;
  ...
  console.log(
    `  Iterations Per Second: ${(metrics.iterations?.values.rate || 0).toFixed(
      2
    )}`
  );
  ...
  console.log("1. Run alongside: docker stats (if app is containerized)");
  console.log("2. Use: k6 run --out json=results.json tc_perf_04.js");
  console.log(
    "3. Integrate with: Prometheus + Grafana for real-time monitoring"
  );
  console.log(
    "4. Server-side: Monitor with htop, top, or performance monitoring tools"
  );
  console.log("=================================================\n");
  ...
  return {
    stdout: JSON.stringify({
      summary: {
        ...
        errorRate: ((metrics.http_req_failed?.values.rate || 0) * 100).toFixed(
          2
        ),
      },
    }),
